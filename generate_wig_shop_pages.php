<?php
// Script to generate localized SEO landing pages for "wig shop in [location]"
// Target structures:
// - wig-shop/delhi/[location]/index.php
// - wig-shop/gurugram/[location]/index.php
...
$delhi_locations = [
    ...
    [
        ...
        'landmarks' => ['DLF Promenade', 'Vasant Square Mall', 'JNU Campus'],
        'metro_route' => 'Take the Magenta Line Metro from Vasant Vihar or Munirka to Janakpuri West, then switch to the Blue Line directly to Dwarka Sector 10 Metro Station. From there, take a short auto-rickshaw ride to Ramphal Chowk.',
        'road_route' => 'Drive via Nelson Mandela Marg, merge onto NH-48 towards Gurgaon/Airport, exit onto the Dwarka Flyover link road, and head straight to Sector 7 Ramphal Chowk.'
    ]
];
...
$gurugram_locations = [
    ...
    [
        'slug' => 'new-gurugram', 'name' => 'New Gurugram',
...
<main class="pt-20">
    ...
    <section class="py-section-gap bg-surface-container-low border-t border-surface-variant/30">
        <div class="max-w-[1280px] mx-auto px-margin-mobile md:px-margin-desktop">
            <h3 class="font-headline-lg text-2xl text-on-surface mb-8 text-center">Other ##STATE_NAME## Locations We Serve</h3>
...
if (!file_exists('wig-shop')) {
    mkdir('wig-shop', 0755);
}
if (!file_exists('wig-shop/delhi')) {
    mkdir('wig-shop/delhi', 0755);
}
if (!file_exists('wig-shop/gurugram')) {
    mkdir('wig-shop/gurugram', 0755);
}

// Prepare Delhi other locs data block
$delhi_other_locs = [];
foreach ($delhi_locations as $l) {
    $delhi_other_locs[] = "                    '{$l['slug']}' => '{$l['name']}'";
}
$delhi_other_locs_str = implode(",\n", $delhi_other_locs);

// Generate Delhi pages
foreach ($delhi_locations as $loc) {
    $dir = 'wig-shop/delhi/' . $loc['slug'];
    if (!file_exists($dir)) {
        mkdir($dir, 0755, true);
    }
    ...
    echo "Generated Delhi wig shop page: " . $loc['name'] . " in " . $dir . "/index.php\n";
}

// Prepare Gurugram other locs data block
$gurugram_other_locs = [];
foreach ($gurugram_locations as $l) {
    $gurugram_other_locs[] = "                    '{$l['slug']}' => '{$l['name']}'";
}
$gurugram_other_locs_str = implode(",\n", $gurugram_other_locs);

// Generate Gurugram pages
foreach ($gurugram_locations as $loc) {
    $dir = 'wig-shop/gurugram/' . $loc['slug'];
    if (!file_exists($dir)) {
        mkdir($dir, 0755, true);
    }
    ...
    file_put_contents($dir . '/index.php', $content);
    echo "Generated Gurugram wig shop page: " . $loc['name'] . " in " . $dir . "/index.php\n";
}

echo "All 30 Wig Shop location landing pages generated successfully!\n";
?>
